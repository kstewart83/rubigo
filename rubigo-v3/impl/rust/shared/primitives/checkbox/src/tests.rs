...
fn dispatch_event(comp: &mut Checkbox, event: &str) {
    match event {
            "SETCHECKED" => { let _ = comp.set_checked(); }
            "SETUNCHECKED" => { let _ = comp.set_unchecked(); }
            "SETINDETERMINATE" => { let _ = comp.set_indeterminate(); }
