...
fn dispatch_event(comp: &mut Tabs, event: &str) {
    match event {
            "SETSELECTED" => { let _ = comp.set_selected(); }
            "FOCUSNEXTTAB" => { let _ = comp.focus_next_tab(); }
            "FOCUSPREVTAB" => { let _ = comp.focus_prev_tab(); }
            "FOCUSFIRSTTAB" => { let _ = comp.focus_first_tab(); }
            "FOCUSLASTTAB" => { let _ = comp.focus_last_tab(); }
            "ACTIVATEFOCUSED" => { let _ = comp.activate_focused(); }
