...
fn dispatch_event(comp: &mut Slider, event: &str) {
    match event {
            "INCREMENTVALUE" => { let _ = comp.increment_value(); }
            "DECREMENTVALUE" => { let _ = comp.decrement_value(); }
            "SETTOMIN" => { let _ = comp.set_to_min(); }
            "SETTOMAX" => { let _ = comp.set_to_max(); }
            "STARTDRAG" => { let _ = comp.start_drag(); }
            "ENDDRAG" => { let _ = comp.end_drag(); }
            "UPDATEVALUE" => { let _ = comp.update_value(); }
