...
fn main() {
    ...
    for (name, spec_path) in &components {
        ...
        let machine_json = if cue_available {
            generate_with_cue_cli(name, &cue_blocks, &out_dir)
                .unwrap_or_else(|e| {
                    println!("cargo:warning=cue export failed for {}: {}, falling back to basic parser", name, e);
                    generate_machine_json_fallback(name, &cue_blocks)
                })
        } else {
            println!("cargo:warning=cue CLI not found, using basic parser for {}", name);
            generate_machine_json_fallback(name, &cue_blocks)
        };
        ...
        println!("cargo:warning=Generated {} from {}", json_path.display(), spec_path.display());
    }
                        ...
                        if file_path.extension().map(|e| e == "md").unwrap_or(false) {
                            if let Some(stem) = file_path.file_stem() {
                                ...
                                if stem_str.ends_with(".sudo") {
                                    let component_name = stem_str.trim_end_matches(".sudo").to_string();
...
fn generate_with_cue_cli(
    ...
    let order = ["context_schema", "state_machine", "guards", "actions", "events_emitted", "invariants"];
