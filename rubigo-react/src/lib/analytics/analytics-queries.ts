...
export async function getErrorRateByRoute(
    ...
    WHERE created_at >= ${interval}
      AND kind = 1  -- Server spans only
    GROUP BY name
    HAVING COUNT(*) >= 5  -- Only routes with enough traffic
...
export async function getOverviewStats(
    ...
    SELECT 
      ...
      ROUND(COUNT(*) FILTER (WHERE status_code = 2) * 100.0 / NULLIF(COUNT(*), 0), 2) AS error_rate,
      ROUND(AVG(duration_ns / 1000000.0), 2) AS avg_response_time
    FROM spans, sessions
