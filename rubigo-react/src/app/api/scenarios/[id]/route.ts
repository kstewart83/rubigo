/**
 * Scenarios REST API - Dynamic Route
 */
...
export async function GET(request: NextRequest, { params }: RouteParams) {
    ...
    try {
        const results = await db.select().from(schema.scenarios).where(eq(schema.scenarios.id, id)).limit(1);
        if (results.length === 0) {
        ...
        return NextResponse.json({ success: false, error: "Failed to get scenario" }, { status: 500 });
    }
}
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
    ...
    const existing = await db.select().from(schema.scenarios).where(eq(schema.scenarios.id, id)).limit(1);
