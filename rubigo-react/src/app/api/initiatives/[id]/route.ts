/**
 * Initiatives REST API - Dynamic Route
 */
...
export async function GET(request: NextRequest, { params }: RouteParams) {
    ...
    try {
        const results = await db.select().from(schema.initiatives).where(eq(schema.initiatives.id, id)).limit(1);
        if (results.length === 0) {
            return NextResponse.json({ success: false, error: "Initiative not found" }, { status: 404 });
        }
        return NextResponse.json({ success: true, data: results[0] });
    } catch (error) {
        console.error("GET initiative error:", error);
        return NextResponse.json({ success: false, error: "Failed to get initiative" }, { status: 500 });
    }
}
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
    ...
    const existing = await db.select().from(schema.initiatives).where(eq(schema.initiatives.id, id)).limit(1);
