/**
 * Metrics REST API - Dynamic Route
 */
...
export async function GET(request: NextRequest, { params }: RouteParams) {
    ...
    try {
        const results = await db.select().from(schema.metrics).where(eq(schema.metrics.id, id)).limit(1);
        if (results.length === 0) {
        ...
        return NextResponse.json({ success: false, error: "Failed to get metric" }, { status: 500 });
    }
}
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
    ...
    const existing = await db.select().from(schema.metrics).where(eq(schema.metrics.id, id)).limit(1);
    ...
    try {
        ...
        return NextResponse.json({ success: false, error: "Failed to update metric" }, { status: 500 });
    }
}
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
    ...
    try {
        ...
        return NextResponse.json({ success: false, error: "Failed to delete metric" }, { status: 500 });
    }
}
