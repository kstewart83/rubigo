/**
 * Activitys REST API - Dynamic Route
 */
...
export async function GET(request: NextRequest, { params }: RouteParams) {
    ...
    try {
        const results = await db.select().from(schema.activities).where(eq(schema.activities.id, id)).limit(1);
        if (results.length === 0) {
        ...
        return NextResponse.json({ success: false, error: "Failed to get activity" }, { status: 500 });
    }
}
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
    ...
    const existing = await db.select().from(schema.activities).where(eq(schema.activities.id, id)).limit(1);
