/**
 * Specifications REST API - Dynamic Route
 */
...
async function logAction(
    ...
    await db.insert(schema.actionLogs).values({
        ...
        operationId: `${action}Specification`,
...
export async function GET(request: NextRequest, { params }: RouteParams) {
    ...
    try {
        const results = await db.select().from(schema.specifications).where(eq(schema.specifications.id, id)).limit(1);
        if (results.length === 0) {
        ...
        return NextResponse.json({ success: false, error: "Failed to get specification" }, { status: 500 });
    }
}
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
    ...
    const existing = await db.select().from(schema.specifications).where(eq(schema.specifications.id, id)).limit(1);
    ...
    try {
        ...
        console.error("PUT specification error:", error);
