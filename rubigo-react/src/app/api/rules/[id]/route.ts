...
export async function GET(request: NextRequest, { params }: RouteParams) {
    ...
    try {
        const results = await db.select().from(schema.rules).where(eq(schema.rules.id, id)).limit(1);
        if (results.length === 0) {
        ...
        return NextResponse.json({ success: false, error: "Failed to get rule" }, { status: 500 });
    }
}
