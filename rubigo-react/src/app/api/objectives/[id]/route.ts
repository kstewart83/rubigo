/**
 * Objectives REST API - Dynamic Route
 */
...
export async function GET(request: NextRequest, { params }: RouteParams) {
    ...
    try {
        const results = await db.select().from(schema.objectives).where(eq(schema.objectives.id, id)).limit(1);
        if (results.length === 0) {
        ...
        return NextResponse.json({ success: false, error: "Failed to get objective" }, { status: 500 });
    }
}
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
    ...
    const existing = await db.select().from(schema.objectives).where(eq(schema.objectives.id, id)).limit(1);
    ...
    try {
        ...
        console.error("PUT objective error:", error);
        return NextResponse.json({ success: false, error: "Failed to update objective" }, { status: 500 });
    }
}
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
    ...
    try {
        ...
        return NextResponse.json({ success: false, error: "Failed to delete objective" }, { status: 500 });
    }
}
